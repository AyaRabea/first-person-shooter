...
namespace Graphics._3D_Models
{
    ...
    class md2LOL 
    {
        ...
        public void attackorrun(md2LOL m,float r) 
        {
            if (r > 25 && r < 80)
            {
                if (m.animSt.type != _3D_Models.animType_LOL.RUN)
                    m.StartAnimation(_3D_Models.animType_LOL.RUN);
                
            }
            else if (r > 20 && r < 30)
            {
                if (m.animSt.type != _3D_Models.animType_LOL.ATTACK1)
                    m.StartAnimation(_3D_Models.animType_LOL.ATTACK1);

            }
            else
            {
                if (m.animSt.type != _3D_Models.animType_LOL.STAND)
                    m.StartAnimation(_3D_Models.animType_LOL.STAND);
            }
        ...
        public void draw(md2LOL m, int matID)
        {
            m.Draw(matID);
        }
        public bool will_die(vec3 bull_pos)
        {
